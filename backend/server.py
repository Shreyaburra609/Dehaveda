...
def gate(items, user, key="premium"):
    """Return items with premium bodies removed for non-premium users, keeping identity fields."""
    premium = bool(user and is_premium_active(user))
    out = []
    for it in items:
        if it.get(key) and not premium:
...
async def jala(user: Optional[dict] = Depends(get_optional_user)):
    types_, premium = gate(C.WATER_TYPES, user)
    params, _ = gate(C.WATER_PARAMETERS, user)
    return {
        "water_types": types_,
...
async def swara(user: Optional[dict] = Depends(get_optional_user)):
    swaras, premium = gate(C.SWARAS, user)
    return {
...
async def manas(user: Optional[dict] = Depends(get_optional_user)):
    topics, premium = gate(C.MANAS_TOPICS, user)
    return {
        "topics": topics,
...
async def submit_score(body: ScoreIn, user: dict = Depends(get_current_user)):
    ...
    if game["premium"] and not is_premium_active(user):
