    ...
    <>
      ...
      <header className="dv-aurora border-b border-slate-800/70">
        <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8 lg:py-20">
          <p className="font-data mb-4 text-[11px] uppercase tracking-[0.3em] text-sky-400">02 — Jala</p>
                ...
                t.locked ? (
                  ...
                  <Reveal key={t.name} delay={Math.min(i, 6) * 50}>
                    <article data-testid={`water-type-${i}`} className="dv-surface h-full rounded-2xl p-6">
                      <Droplets className="mb-4 h-5 w-5 text-sky-400" />
                      <h3 className="font-display text-lg font-semibold text-slate-50">{t.name}</h3>
                      <p className="mt-3 text-sm leading-relaxed text-slate-400">{t.summary}</p>
                    </article>
                  </Reveal>
                ),
              )}
                ...
                {data.journey.map((s, i) => (
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-full border px-4 py-2 text-xs transition-colors ${
                      step === i ? "border-sky-500/60 bg-sky-500/12 text-sky-300" : "border-slate-700 text-slate-400 hover:text-slate-200"
                    }`}
                  >
                    <span className="font-data text-[10px] text-slate-500">{s.step}</span>
                    {s.title}
                    {i < data.journey.length - 1 && <ChevronRight className="h-3 w-3 text-slate-600" />}
                  </button>
                ))}
              </div>
              <div data-testid="journey-detail" className="dv-surface mt-8 grid gap-6 rounded-3xl p-8 lg:grid-cols-2">
                <div>
                  <p className="font-data text-[10px] uppercase tracking-[0.2em] text-sky-400">
                    Stage {data.journey[step].step} of {data.journey.length}
                  </p>
                  <h3 className="font-display mt-3 text-2xl font-bold text-slate-50">{data.journey[step].title}</h3>
                  <p className="mt-4 text-sm leading-relaxed text-slate-400">{data.journey[step].description}</p>
                </div>
                <div className="rounded-2xl border border-amber-500/25 bg-amber-500/5 p-6">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-amber-100/80">{data.journey[step].risk}</p>
                </div>
              </div>
            </div>
          </section>
                ...
                {data.parameters.map((p, i) => (
                  <button
                    ...
                    className={`flex items-center justify-between rounded-xl border px-4 py-3 text-left text-sm transition-colors ${
                      param === i ? "border-sky-500/50 bg-sky-500/10 text-sky-200" : "border-slate-800 text-slate-400 hover:text-slate-200"
                    }`}
                  >
                    {p.name}
                    {p.locked && <span className="font-data text-[9px] uppercase tracking-wider text-amber-400">Premium</span>}
                  </button>
                ))}
              </div>
              <div className="lg:col-span-8">
                {activeParam?.locked ? (
                  ...
                  <div data-testid="param-detail" className="dv-surface rounded-3xl p-8">
                    <h3 className="font-display text-2xl font-bold text-slate-50">{activeParam.name}</h3>
                    ...
                    <div className="mt-7 rounded-2xl border border-sky-500/25 bg-sky-500/5 p-5">
                      <p className="font-data text-[10px] uppercase tracking-[0.18em] text-sky-300">Reference values</p>
                      <p className="mt-2 text-sm leading-relaxed text-sky-100/85">{activeParam.reference}</p>
                    </div>
                  </div>
                )}
              </div>
                ...
                {data.contamination.map((c, i) => (
                  <Reveal key={c.type} delay={Math.min(i, 6) * 50}>
                    <article data-testid={`contamination-${i}`} className="dv-surface h-full rounded-2xl p-6">
                      <h3 className="font-display text-lg font-semibold text-slate-50">{c.type}</h3>
                      <p className="mt-3 text-sm leading-relaxed text-slate-400">{c.detail}</p>
                      <p className="mt-4 rounded-xl bg-emerald-500/8 px-4 py-3 text-xs leading-relaxed text-emerald-200/85">
