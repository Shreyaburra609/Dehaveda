...
const GALLERY = [
  ...
  { url: "https://images.pexels.com/photos/7519797/pexels-photo-7519797.jpeg?auto=compress&cs=tinysrgb&w=1400", caption: "Swara — sound, vibration and voice", alt: "Musician performing with a stringed instrument" },
  { url: "https://images.unsplash.com/photo-1627728724901-e79f35800820?crop=entropy&cs=srgb&fm=jpg&q=80&w=1400", caption: "Manas — attention, memory and calm", alt: "Person sitting quietly in a calm natural setting" },
...
function Counter({ value, testid }) {
  ...
  return (
    <span data-testid={testid} className="font-display text-4xl font-bold text-slate-50 sm:text-5xl">
...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="dv-grain dv-aurora relative overflow-hidden border-b border-slate-800/70">
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-20 lg:grid-cols-12 lg:gap-8 lg:px-8 lg:py-28">
          <div className="dv-rise lg:col-span-7">
            <p className="font-data mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-500/25 bg-emerald-500/8 px-3.5 py-1.5 text-[10px] uppercase tracking-[0.28em] text-emerald-300">
              Five Pillars · One Ecosystem
            </p>
            <h1 className="font-display text-4xl font-extrabold leading-[1.05] tracking-tight text-slate-50 sm:text-5xl lg:text-6xl">
            ...
            <p className="mt-7 max-w-xl text-sm leading-relaxed text-slate-400 sm:text-base">
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              <a href="#pillars">
                <Button data-testid="hero-explore-button" size="lg" className="rounded-full bg-emerald-500 px-7 text-slate-950 hover:bg-emerald-400">
              ...
              <Link to="/games">
                <Button data-testid="hero-games-button" size="lg" variant="ghost" className="rounded-full px-7 text-slate-300">
                  Play Games
                </Button>
              </Link>
            </div>
          ...
          <div className="dv-rise lg:col-span-5" style={{ animationDelay: "160ms" }}>
            <div className="relative overflow-hidden rounded-[28px] border border-slate-800">
              <img
              ...
              <div className="dv-glass absolute bottom-4 left-4 right-4 rounded-2xl px-5 py-4">
                <p className="font-data text-[10px] uppercase tracking-[0.24em] text-emerald-300">Live community</p>
          ...
          {PILLARS.map((p, i) => (
            <Reveal key={p.code} delay={i * 80}>
              <Link
                ...
                className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-slate-800 bg-[#0f1626] transition-all duration-500 hover:-translate-y-1.5 hover:border-slate-700"
              >
                <div className="relative h-44 overflow-hidden">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0f1626] via-[#0f1626]/35 to-transparent" />
                  <span className="font-data absolute left-5 top-5 text-[11px] tracking-[0.3em]" style={{ color: p.accent }}>
                    {p.index}
                  </span>
                </div>
                <div className="flex flex-1 flex-col p-6">
                  <h3 className="font-display text-2xl font-bold tracking-tight text-slate-50">{p.name}</h3>
                  <p className="mt-1 text-xs font-medium" style={{ color: p.accent }}>
                    {p.subtitle}
                  </p>
                  <p className="mt-4 flex-1 text-sm leading-relaxed text-slate-400">{p.blurb}</p>
                  <span className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold text-slate-200 transition-colors group-hover:text-emerald-300">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {HIGHLIGHTS.map((h, i) => (
              <Reveal key={h.title} delay={i * 60}>
                ...
                >
                  <h.icon className="mb-4 h-5 w-5 text-emerald-400" />
                  <p className="font-display text-lg font-semibold text-slate-100">{h.title}</p>
            ...
            {[
              ...
              <Reveal key={s.key} delay={i * 70}>
                <div className="dv-surface rounded-2xl p-7">
                  <s.icon className="mb-5 h-5 w-5 text-emerald-400" />
                  {stats ? <Counter value={stats[s.key]} testid={s.testid} /> : <span className="font-display text-4xl text-slate-700">—</span>}
          ...
          <div className="grid gap-10 lg:grid-cols-12">
            <div className="lg:col-span-5">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link to="/membership">
                  <Button data-testid="home-subscribe-button" className="rounded-full bg-amber-500 px-6 text-slate-950 hover:bg-amber-400">
                    Subscribe
                  </Button>
                </Link>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:col-span-7">
              {plans.map((plan) => (
                ...
                >
                  <p className="font-display text-xl font-semibold text-slate-50">{plan.name}</p>
                  <p className="font-data mt-3 text-2xl text-emerald-300">
      ...
      <section id="ai-assistant" className="mx-auto max-w-7xl px-4 py-20 lg:px-8">
        <div className="dv-glass dv-grain relative overflow-hidden rounded-3xl p-8 sm:p-14">
          <Sparkles className="mb-6 h-6 w-6 text-emerald-400" />
          <h2 className="font-display max-w-2xl text-2xl font-bold tracking-tight text-slate-50 sm:text-3xl lg:text-4xl">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {["How many calories are in an apple?", "What is groundwater?", "What is Sa in Swara?"].map((q) => (
              <span key={q} className="font-data rounded-full border border-slate-700 px-3.5 py-2 text-[11px] text-slate-400">
                {q}
              </span>
            ))}
          </div>
