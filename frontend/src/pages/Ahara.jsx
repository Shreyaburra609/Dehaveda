      ...
      <form onSubmit={submit} data-testid="calorie-form" className="dv-surface rounded-3xl p-7">
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-xs text-slate-400">
            Age (years)
            <Input data-testid="calorie-age" type="number" min="10" max="100" value={form.age} onChange={set("age")} className="mt-2 bg-slate-900/60" required />
          </label>
          <label className="text-xs text-slate-400">
            Sex
            <select data-testid="calorie-sex" value={form.sex} onChange={set("sex")} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-200">
          ...
          <label className="text-xs text-slate-400">
            Height (cm)
            <Input data-testid="calorie-height" type="number" min="90" max="250" value={form.height_cm} onChange={set("height_cm")} className="mt-2 bg-slate-900/60" required />
          </label>
          <label className="text-xs text-slate-400">
            Weight (kg)
            <Input data-testid="calorie-weight" type="number" min="25" max="300" value={form.weight_kg} onChange={set("weight_kg")} className="mt-2 bg-slate-900/60" required />
          </label>
          <label className="text-xs text-slate-400 sm:col-span-2">
            Activity level
            <select data-testid="calorie-activity" value={form.activity} onChange={set("activity")} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-200">
              {ACTIVITY.map(([v, l]) => (
          ...
          <label className="text-xs text-slate-400 sm:col-span-2">
            Goal
            <select data-testid="calorie-goal" value={form.goal} onChange={set("goal")} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-200">
        ...
        <Button data-testid="calorie-submit" type="submit" disabled={busy} className="mt-6 w-full rounded-full bg-emerald-500 text-slate-950 hover:bg-emerald-400">
          {busy ? "Calculating…" : "Calculate my estimate"}
        </Button>
        {error && <p data-testid="calorie-error" className="mt-4 text-xs text-red-400">{error}</p>}
      </form>

      <div className="dv-surface rounded-3xl p-7">
        {result ? (
          <div data-testid="calorie-result">
            <p className="font-data text-[10px] uppercase tracking-[0.2em] text-emerald-400">Your estimate</p>
            <div className="mt-6 grid grid-cols-2 gap-4">
              <div>
                <p className="font-display text-4xl font-bold text-slate-50" data-testid="calorie-bmr">{result.bmr}</p>
                <p className="font-data mt-1 text-[10px] uppercase tracking-[0.16em] text-slate-500">BMR kcal/day</p>
              </div>
              <div>
                <p className="font-display text-4xl font-bold text-emerald-300" data-testid="calorie-maintenance">{result.maintenance}</p>
                <p className="font-data mt-1 text-[10px] uppercase tracking-[0.16em] text-slate-500">Maintenance kcal/day</p>
              </div>
            </div>
            <div className="mt-7 space-y-3">
              <div className="rounded-xl bg-slate-900/60 p-4">
                <p className="text-xs text-slate-400">{result.goal_label} range</p>
                <p className="font-data mt-1 text-lg text-sky-300" data-testid="calorie-goal-range">
            ...
            <p className="mt-6 flex gap-2 text-[11px] leading-relaxed text-amber-300/80">
              <Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /> {result.disclaimer}
            </p>
            <p className="font-data mt-3 text-[10px] text-slate-600">{result.formula}</p>
          </div>
        ) : (
          <div className="flex h-full flex-col justify-center">
            <Flame className="mb-4 h-6 w-6 text-amber-400" />
            <p className="font-display text-xl text-slate-200">Your estimate appears here</p>
    ...
    <>
      ...
      <header className="dv-aurora border-b border-slate-800/70">
        <div className="mx-auto max-w-7xl px-4 py-16 lg:px-8 lg:py-20">
          <p className="font-data mb-4 text-[11px] uppercase tracking-[0.3em] text-amber-400">01 — Ahara</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-8 flex flex-col gap-4">
          <div className="relative max-w-md">
            ...
            <Input
              ...
              className="rounded-full border-slate-700 bg-slate-900/60 pl-11"
            />
          </div>
        ...
        <div className="mt-10">
          ...
          {!loading && !error && items.length > 0 && (
            <>
              {data.locked_count > 0 && (
                <p data-testid="food-locked-notice" className="mb-6 rounded-2xl border border-amber-500/25 bg-amber-500/5 px-5 py-3.5 text-xs text-amber-300">
                    ...
                    <Reveal key={f.id} delay={Math.min(i, 8) * 40}>
                      <article data-testid={`food-card-${f.name.toLowerCase().replace(/\W+/g, "-")}`} className="dv-surface h-full rounded-2xl p-6">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <h3 className="font-display text-xl font-semibold text-slate-50">{f.name}</h3>
                            <p className="mt-1 text-[11px] text-amber-400">{f.category}</p>
                          </div>
                          <div className="text-right">
                            <p className="font-data text-2xl text-emerald-300">{f.calories}</p>
                            <p className="font-data text-[9px] uppercase tracking-[0.14em] text-slate-500">kcal</p>
                          </div>
                        </div>
                        <p className="font-data mt-4 text-[10px] uppercase tracking-[0.14em] text-slate-600">{f.serving_size}</p>
                        ...
                        {f.micronutrients && (
                          <p className="mt-4 text-xs text-sky-300/90">{f.micronutrients}</p>
                        )}
                        {f.note && <p className="mt-3 text-xs leading-relaxed text-slate-400">{f.note}</p>}
                      </article>
                    </Reveal>
                  ),
                )}
